...
namespace GAME_NS {
  ...
  private:
    ...
    // Checks if a given Board Description is a repeat of a previous state to prevent getting stuck in a loop
    ...
    // The number of moves we have considered (Note: if we find that a move causes two queens to occupy the same cell or causes it to move off the board, we stop early and do not consider this as an evaluation)
